...
export default function Page() {
  ...
  const displayedAvailability = useMemo(() => {
    ...
    return activeCalendarView in availability ? { [activeCalendarView]: availability[activeCalendarView] } : {};
  }, [activeCalendarView, availability]);

  const displayedParticipants = useMemo(() => (activeCalendarView === "all" ? participants : participants.filter((name) => name === activeCalendarView)), [activeCalendarView, participants]);
  ...
  const majoritySlots = useMemo(() => {
    ...
    weekDays.forEach((day) => {
      const dateKey = formatDateKey(day);
      TIME_SLOTS.forEach((slot) => {
        const count = (availability ? Object.values(availability).filter((personData) => (personData[dateKey] || []).includes(slot.id)).length : 0);
        if (count > threshold) {
          result.push({ dateKey, slotId: slot.id, label: slot.label, count });
        }
      });
  ...
  }, [availability, participants.length, weekDays]);
    ...
    <div className="min-h-screen bg-[#f6f7f4] p-3 sm:p-4 lg:p-6">
      <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-4 lg:gap-6">
        ...
        <Card className="overflow-hidden rounded-[28px] border-slate-200 shadow-sm">
          <CardHeader className="gap-3 border-b border-slate-100 pb-4 sm:gap-4 sm:pb-5">
            <div className="flex items-center justify-between gap-3">
              <div className="text-lg font-semibold text-slate-900 sm:text-2xl">{calendarTitle}</div>
              <Button
                className="h-9 gap-2 rounded-[16px] sm:h-11 sm:rounded-[18px] lg:h-14 lg:rounded-[22px] lg:px-6 lg:text-lg"
                onClick={() => void saveMyAvailability()}
                disabled={isSaving || !savedMyName || !isDirty}
              >
                <Save className="h-4 w-4 lg:h-5 lg:w-5" />
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 lg:justify-between">
              ...
              <Button variant="outline" className="h-9 gap-2 rounded-[16px] sm:h-11 sm:rounded-[18px] lg:h-14 lg:rounded-[22px] lg:px-6 lg:text-lg" onClick={() => void fetchAllAvailability(true)} disabled={isRefreshing || isLoading}>
                ...
                {TIME_SLOTS.map((slot) => (
                  <React.Fragment key={slot.id}>
                    ...
                    {weekDays.map((day) => {
                      ...
                      const hasAnySelection = count > 0;
                      const isTopThree = topThreeCellKeys.has(`${dateKey}__${slot.id}`);
                      const totalShown = displayedParticipants.length;
                      ...
                      return (
                        <button
                          ...
                          className={`relative min-h-[52px] rounded-[14px] px-1 py-1 text-left transition sm:min-h-[68px] sm:rounded-[18px] sm:px-1.5 sm:py-1.5 md:min-h-[82px] md:rounded-[20px] lg:min-h-[112px] lg:rounded-[28px] lg:px-4 lg:py-4 ${
                            selected
                              ? isTopThree && activeCalendarView === "all"
                                ? "border-4 border-blue-500 bg-green-600 text-white"
                                : hasAnySelection
                                  ? "border-2 border-blue-500 bg-green-100 text-slate-800"
                                  : "border-2 border-blue-500 bg-white text-slate-800"
                          ...
                          <div className="flex h-full items-end justify-end">
                            {(hasAnySelection || selected) ? (
                              <Badge
                                variant="outline"
                                className={`max-w-full whitespace-nowrap px-1 py-0 text-[9px] font-semibold leading-none sm:px-1.5 sm:py-0.5 sm:text-[11px] md:px-2 md:py-0.5 md:text-xs lg:px-2.5 lg:py-1 lg:text-sm xl:text-base ${
                                  isTopThree && activeCalendarView === "all" ? "border-white/40 bg-white/10 text-white" : "border-slate-200 text-slate-700"
                                }`}
                              >
                                {count} / {totalShown}
                              </Badge>
